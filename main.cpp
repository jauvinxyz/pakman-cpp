...
void drawPacman(float positionX, float positionY, float rotation){
	...
	for (int k = 0; k < 32; k++){
		...
		for (int i = (int)(angle/6); i < (360 - (int)(angle/6)); i++){
			glVertex2f(x, y);
...
void keyOperations(){
	...
	int x1Quadrant = (int)((x - 22.0) / squareSize);
	int x2Quadrant = (int)((x + 22.0) / squareSize);
	int y1Quadrant = (int)((y - 22.0) / squareSize);
	int y2Quadrant = (int)((y + 22.0) / squareSize);
	//update according to keys pressed
	if (keyStates['a']){
		x1Quadrant = (int)((x - 26.0) / squareSize);
		if(rotation != 2){
			rotation = 2;
		} else if (!bitmap.at(x1Quadrant).at(y1Quadrant) && !bitmap.at(x1Quadrant).at(y2Quadrant)){
			xIncrement -= 4 / squareSize;		
			//drawstate = 1;
		}
		keyStates['a'] = false;		
	}
	if (keyStates['d']){
		x2Quadrant = (int)((x + 26.0) / squareSize);
		if(rotation != 0){
			rotation = 0;
	...
	if (keyStates['w']){
		y1Quadrant = (int)((y - 26.0) / squareSize);
		if(rotation != 3){
			rotation = 3;
		} else if (!bitmap.at(x1Quadrant).at(y1Quadrant) && !bitmap.at(x2Quadrant).at(y1Quadrant)){
			yIncrement -= 4 / squareSize;
			//drawstate = 1;			
		}
		keyStates['w'] = false;
	}
	if (keyStates['s']){
		y2Quadrant = (int)((y + 26.0) / squareSize);
		if(rotation != 1){
			rotation = 1;
